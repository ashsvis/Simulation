...
{
    public class GEN : CommonLogic, IEmbededMemory
    {
        public GEN() : base(LogicFunction.Generator, 1)
        {
            SetValueToOut(0, false);
        }

        [Browsable(false)]
        public override string FuncSymbol => "GEN"; // Генератор
        ...
        public override void Calculate()
        {
            ...
            if (!state && timeState < DateTime.Now)
            {
                state = true;
                timeState = DateTime.Now + TimeSpan.FromSeconds(WaitTime / 2);
            }
            SetValueToOut(0, input && state);
            if (state && timeState < DateTime.Now)
            {
                state = false;
                timeState = DateTime.Now + TimeSpan.FromSeconds(WaitTime / 2);
